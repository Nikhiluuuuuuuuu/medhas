...
import medhas.memory.atomic as atomic_mem
import medhas.memory.graph as graph_mem
import medhas.memory.session as session_mem
from medhas.memory.atomic.hashing import content_hash
from medhas.llm.gateway import get_extractor
from medhas.utils import measure_latency, log_atomic, log_graph, log_error
from medhas.pipeline.prompts import EXTRACTION_PROMPT, CONTEXT_TEMPLATE
...
async def extract_and_persist_background(
    ...
    async with measure_latency("pipeline.async_extractor.extract_and_persist_background"):
        try:
            ...
            try:
                response = await get_extractor().chat_completion(messages, temperature=0.0)
