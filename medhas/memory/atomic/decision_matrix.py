...
from medhas.core.exceptions import LLMProviderError
...
async def decide_fact_action(
    ...
    Falls back to a hash/cosine heuristic if the LLM cannot be reached. The LLM is a
    provider-agnostic :class:`medhas.llm.base.BaseLLM` (resolved via the gateway when omitted).
    """
    if not existing:
