...
from medhas.memory.control.memory_types import MemoryType, route, is_valid

# admission (E32)
from medhas.memory.control.admission import evaluate_admission

# bitemporal / belief / provenance (E10-E12)
from medhas.memory.graph.bitemporal import (
...
from medhas.memory.operations.consolidation import (
    compress_episodes, induce_skills, evolve_memory_network, run_consolidation,
)

# scheduler (E7 runtime) + rehearsal (E36)
from medhas.memory.control.scheduler import scheduler, rehearse

# interference / WM eviction / recognition (E5, E35, E37)
from medhas.memory.control.interference import (
...
from medhas.metamemory.metacognitive import (
    route_query, should_abstain, RetrievalOutcome, critic, log_memory_event,
)

# user model + temporal/causal API (E19, E20)
from medhas.memory.operations.usermodel import (
    build_user_model, get_user_model, timeline, what_changed, why_chain,
)

# prospective / sensory / metamemory (E27, E29, E31)
from medhas.memory.control.prospective import (
    add_intention, check_cues, complete_intention, list_intentions,
)
from medhas.metamemory.metamemory import assess as metamemory_assess, known_unknowns, knowledge_map
from medhas.cognition.embodiment import BodyModel  # cognition subsystem (forward-ref in think())
from medhas.platform.sensory import buffer_percept, promote_percepts, sweep_expired, list_buffer, attention_filter

# ingest / chunking / extraction (E4)
from medhas.memory.operations.ingest import chunk_text, extract_facts, extract_from_document, detect_duplicates

# auth / rate-limit (E22, E23)
from medhas.platform.auth import authenticate, authorize, rate_limiter

# scaling / export (E25, E26)
from medhas.memory.control.scaling import ensure_hot_view, refresh_hot_view, partition_report
from medhas.platform.export import export_user_memory, export_to_file, import_user_memory

# eval (E21)
from medhas.platform.eval import run_eval_suite, temporal_consistency_check, EvalCase
...
class MemoryEngine:
    ...
    async def remember(
        ...
        *,
        ...
        async with measure_latency("engine.remember"):
            ...
            try:
                from medhas.extraction.graph_build import build_fact_graph
        ...
        async with measure_latency("engine.recall"):
            ...
            if valid_at is not None:
                ...
                if temporal:
                    ...
                    from medhas.extraction.anaphora import resolve_query_entities
                    from medhas.extraction.entities import query_entities
                    ...
                    try:
                        ...
                        if not resolved and re.search(r"\b(he|she|they|his|her|their)\b", query.lower()):
                            try:
                                from medhas.extraction.anaphora import _most_salient_person
                        ...
                        if relational:
                            ...
                            if extra:
                                ...
                                # convert back.
                                try:
                                    from medhas.extraction.multihop import rank_for_query
    ...
    async def think(self, text: str, user_id: str, *, modality: str = "text",
        ...
        """
        from medhas.cognition import cognitive_step
