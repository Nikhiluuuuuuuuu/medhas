...
class MedhasMemoryCore:
    def __init__(self, db_path: str = "./medhas_db", wal_path: str = "./medhas_wal.db"):
        ...
        self.canonicalizer = EntityCanonicalizer(model_name='paraphrase-multilingual-MiniLM-L12-v2')
            ...
            if not df.empty:
                for _, row in df.iterrows():
                    if row['e.embedding'] is not None:
                        self.vector_index.add_node(row['e.id'], row['e.embedding'])
        except Exception:
            pass

    def remember_raw_text(self, raw_text: str, modality: str = "text", custom_llm_fn = None) -> list:
        """
        Uses dynamic Multilingual AI extraction to convert any raw text paragraph in ANY 
        language (Hindi, Spanish, French, German, Chinese, Japanese, English, etc.) into 
